...
export default async function SalesPage({ searchParams }: { searchParams: Promise<{ vista?: string }> }) {
  const showCancelled = (await searchParams).vista === "canceladas";
  const data = await getSalesDashboard(showCancelled);
  return (
    <main className="shell py-10">
      ...
      {!data.configured ? <p className="card mt-8 p-6 text-[var(--muted)]">Configurá Neon y Clerk para operar ventas reales.</p> : (
        <>
          {!showCancelled && <section className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          {showCancelled && <div className="mt-8"><h2 className="text-2xl font-bold">Ventas canceladas</h2><p className="mt-2 text-sm text-[var(--muted)]">Se conservan separadas para mantener el historial y la auditoría.</p></div>}
