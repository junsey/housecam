...
export default async function SaleDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="shell py-10">
    ...
    <section className="card mt-8 p-6"><h2 className="text-xl font-bold">Productos</h2>
      ...
      {editable && <form action={addSaleItemAction} className="mt-6 grid gap-3 lg:grid-cols-[2fr_1fr_1fr_1fr_auto]"><input name="saleId" type="hidden" value={sale.id} /><select className={field} name="productId" required><option value="">Producto</option>{productOptions.map((product) => <option value={product.id} key={product.id}>{product.name} · {product.sku} · {product.storefront}</option>)}</select><select className={field} name="purchaseMode"><option value="unit">Unidad</option><option value="pack10">Pack 10</option></select><input className={field} min="1" name="quantity" placeholder="Cantidad" type="number" required /><input className={field} min="0" name="finalUnitPriceCents" placeholder="Precio final opcional" type="number" /><button className="rounded-xl border border-[var(--border)] px-4 font-bold">Agregar</button></form>}
    </section>
